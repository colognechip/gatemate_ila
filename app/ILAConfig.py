...
class ClockSource(IntEnum):
    EXT_INPUT = 1            # external clk input
...
def execute_tool(command, output_file, output_log, log = False):
    try:
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, text=True)
        output, error = process.communicate()
        if log:
            with io.open(output_log, 'w', encoding='utf-8') as logger:
                logger.write("\ncommand: \n")
        ...
        if not os.path.exists(output_file):
            print(
                ...
                error + os.linesep + os.linesep + "log:")
...
class ILAConfig:
    ...
    def __init__(self, __version__):
        ...
        self.clk_state: ClockSource | None = None
    ...
    def choose_clk_source(self, found_pll):
        ...
        while True:
            ...
            self.clk_state = ClockSource.AUX_PLL_FREE
            ...
            try:
                ...
                if selection == 1 or selection == 2:
                    ...
                    if not in_state:
                        ...
                        self.clk_state = ClockSource.DESIGN_PLL_DERIVED
    ...
    def choose_external_clk(self):
        ...
        while True:
            ...
            if response == 'n':
                ...
                self.clk_state = ClockSource.EXT_INPUT
    ...
    def create_DUT(self, code_lines, found_element):
        ...
        if self.clk_state == ClockSource.DESIGN_PLL_DERIVED:
    ...
    def set_config_ILA(self):
        ...
        assign_clk = ""
        if (self.clk_state == ClockSource.AUX_PLL_FREE):
            assign_clk = "assign ILA_clk_src = " + self.clk_name + ";" + os.linesep
            assign_clk += "assign DUT_clk = " + self.clk_name + ";" + os.linesep
        if not (self.clk_state == ClockSource.DESIGN_PLL_DERIVED):
            connect_clk = ""
        else:
        ...
        for single_signal in self.ports_DUT:
            if self.input_ctrl and single_signal["Signal_name"] == self.input_ctrl_signal_name:
                ...
                if (self.clk_state == ClockSource.AUX_PLL_FREE and single_signal["Signal_name"] == self.clk_name):
                    insert_str += "(* clkbuf_inhibit *) " + out_signal(single_signal) + "," + os.linesep
                    instance_of_dut += "." + single_signal["Signal_name"] + "(DUT_clk_global), "
                    continue
                else:
        ...
        with open('..'+ os.path.sep +'src'+ os.path.sep +'ILA_top.v', "r") as file:
            ...
            content = content.replace(
                ...
                f"parameter USE_PLL = {str(int(self.clk_state == ClockSource.AUX_PLL_FREE))}")
            content = content.replace(
            ...
            if self.clk_state == ClockSource.EXT_INPUT:
    ...
    def executing_toolchain(self):
        try:
            ...
            if USE_NEXTPNR:
                output_file_yosys = save_gl_dir + os.path.sep + 'net' + os.path.sep + 'ila_top_synth' + self.time_stamp + '.json'
                yosys_command = (YOSYS + ' -ql ' + log_file + ' -p "read_verilog -sv ' + all_files + \
                            '; synth_gatemate -top ila_top ' + YOSYS_FLAGS_NEXTPNR + '; write_json ' + output_file_yosys + "; " +
                                 YOSYS_OPTIONAL_WRITE_NEXTPNR+'"')
            else:
            ...
            if USE_NEXTPNR:
                output_file_npnr = output_file_p_r + '.txt'
                nexpnr_command = NEXTPNR + ' ' + NEXTPNR_FLAGS + ' --json ' + output_file_yosys +  " -o ccf=" + \
                                 ccf_file_ila_source + " -o out=" + output_file_npnr + ' --router router2 '
                if not execute_tool(nexpnr_command, output_file_npnr, log_file, True):
                    return False
                self.toolchain_info = output_file_p_r + '.bit'
                command = GMPACK + " " + output_file_npnr + " " + self.toolchain_info
                if not execute_tool(command, self.toolchain_info, ""):
                    return False
    ...
    def upload(self):
        ...
        while True:
            ...
            upload_cmd = UPLOAD + " " + UPLOAD_FLAGS + " " + self.toolchain_info
            process = subprocess.Popen(upload_cmd, stderr=subprocess.PIPE,
